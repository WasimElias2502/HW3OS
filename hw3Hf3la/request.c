...
void requestHandle(int fd ,struct timeval arrival , struct timeval dispatch, threads_stats myStats)
{
   ...
   if (strcasecmp(method, "GET")) {
       requestError(fd, method, "501", "Not Implemented", "OS-HW3 Server does not implement this method", arrival, dispatch, myStats);
      return;
   }
   requestReadhdrs(&rio);
   ...
   if (stat(filename, &sbuf) < 0) {
       requestError(fd, filename, "404", "Not found", "OS-HW3 Server could not find this file", arrival, dispatch, myStats);
      return;
   }

   if (is_static) {
      if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) {
          requestError(fd, filename, "403", "Forbidden", "OS-HW3 Server could not read this file", arrival, dispatch, myStats);
         return;
      }
       (myStats->stat_req)++;
      requestServeStatic(fd, filename, sbuf.st_size,arrival,dispatch,myStats);
   } else {
      if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) {
          requestError(fd, filename, "403", "Forbidden", "OS-HW3 Server could not run this CGI program", arrival, dispatch, myStats);
         return;
      }
       (myStats->dynm_req)++;
      requestServeDynamic(fd, filename, cgiargs, arrival,dispatch,myStats);
   }
}
